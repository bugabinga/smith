...
const ADW_OPERATIONAL_WORKFLOWS = Object.freeze([
  ...
  Object.freeze({ file: "adw-operations.yml", name: "ADW maintenance triggers", path: ".github/workflows/adw-operations.yml", events: Object.freeze(["push", "repository_dispatch", "schedule", "workflow_dispatch"]) }),
...
export function createGitHub({ repository, token, appIdentity, ghPath, run = runProcess, baseEnv, sleep = milliseconds => new Promise(resolve => setTimeout(resolve, milliseconds)), now = () => new Date().toISOString() }) {
  ...
  const runEntityId = (workflow, displayTitle, repositoryId) => {
    ...
    if (workflow?.file === "adw-operations.yml" && /^ADW maintenance (?:audit|push|reconcile|[^\s]+(?: [^\s]+){4})$/.test(displayTitle ?? "")) return repositoryId;
    return null;
  };
