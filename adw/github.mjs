...
export function createGitHub({ repository, token, appIdentity, ghPath, run = runProcess, baseEnv, sleep = milliseconds => new Promise(resolve => setTimeout(resolve, milliseconds)), now = () => new Date().toISOString() }) {
  ...
  const methods = {
    ...
    commitFiles: mergeSha => {
      contract(typeof mergeSha === "string" && /^[0-9a-f]{40}$/.test(mergeSha), "merge SHA is invalid");
      return request(`/repos/${owner}/${name}/commits/${mergeSha}`, "files", { pageSize: MAX_PULL_FILE_ITEMS, maxItems: MAX_PULL_FILE_ITEMS, maxBytes: MAX_COLLECTION_BYTES });
    },
  ...
  const changedFiles = pull => {
    if (!pull.merged) return methods.pullFiles(Number(pull.number));
    if (!pull.mergeSha) throw new AdwError("forge", "merged pull lacks merge SHA");
    return methods.commitFiles(pull.mergeSha);
  };
  const enrichPull = async (raw, files = null, maintenance = false, fullAudit = false, mergedDetails = true) => {
    ...
    const rawFiles = includeDetails ? files ?? await changedFiles(pull) : [];
  ...
  const api = {
    ...
    async readRoleSnapshot(event, rolePolicy, { controlSha, appId } = {}) {
      ...
      if (["pull", "diff", "files", "reviews", "security", "changed_paths", "findings", "docs", "dependency"].some(field => fields.has(field)) && pullRelated) {
        ...
        const rawFiles = await changedFiles(normalizedPull);
