...
const ADW_OPERATIONAL_WORKFLOWS = Object.freeze([
  Object.freeze({ file: "adw-issues.yml", name: "ADW issue and reusable execution lanes", path: ".github/workflows/adw-issues.yml", events: Object.freeze(["issue_comment", "issues", "repository_dispatch"]) }),
  Object.freeze({ file: "adw-maintenance.yml", name: "ADW maintenance triggers", path: ".github/workflows/adw-maintenance.yml", events: Object.freeze(["push", "repository_dispatch", "schedule", "workflow_dispatch"]) }),
  Object.freeze({ file: "adw-pulls.yml", name: "ADW pull and reconcile triggers", path: ".github/workflows/adw-pulls.yml", events: Object.freeze(["check_run", "check_suite", "pull_request_review", "pull_request_review_comment", "pull_request_target", "repository_dispatch"]) }),
]);
...
export function createGitHub({ repository, token, appIdentity, ghPath, run = runProcess, baseEnv, sleep = milliseconds => new Promise(resolve => setTimeout(resolve, milliseconds)), now = () => new Date().toISOString() }) {
  ...
  const runEntityId = (workflow, displayTitle, repositoryId) => {
    let match;
    if (workflow?.file === "adw-issues.yml" && (match = /^ADW issue #([1-9][0-9]*)$/.exec(displayTitle ?? ""))) return match[1];
    if (workflow?.file === "adw-pulls.yml" && (match = /^ADW pull #([1-9][0-9]*)$/.exec(displayTitle ?? ""))) return match[1];
    if (workflow?.file === "adw-maintenance.yml" && /^ADW maintenance (?:audit|push|reconcile|[^\s]+(?: [^\s]+){4})$/.test(displayTitle ?? "")) return repositoryId;
    return null;
  };
  const normalizeRun = (value, repositoryId = null, evidence = null) => {
    ...
    const workflow = ADW_OPERATIONAL_WORKFLOWS.find(candidate => candidate.path === value.path);
    return Object.freeze({
      ...
      event: text(value.event, "run event"), entityId: repositoryId === null || value.event === "repository_dispatch" ? null : runEntityId(workflow, value.display_title, repositoryId),
  ...
  const cancelledApplyEvidence = async (workflow, value, { repositoryId, defaultBranch, controlSha }) => {
    ...
    if (value?.name !== workflow.name || value?.path !== workflow.path || !workflow.events.includes(value?.event)) return null;
    ...
    contract(baseRun.entityId !== null, "operational run entity is malformed");
    let runControlSha;
