...
function mergedRoleGitHub(commitFiles = mergedPullFiles.commitFiles.body.files) {
  const endpoints = [];
  ...
  const github = adapter(async request => {
    ...
    if (endpoint === commitEndpoint) return reply({ ...mergedPullFiles.commitFiles.body, files: commitFiles });
  ...
  return { github, endpoints, commitEndpoint };
}

test("merged docs snapshots use bounded immutable merge-commit files and exact merge authority", async () => {
  const { github, endpoints, commitEndpoint } = mergedRoleGitHub();
  ...
  const expectedPaths = mergedPullFiles.commitFiles.body.files.map(file => file.filename).sort();
  assert.deepEqual(snapshot.state.changedPaths, expectedPaths);
  assert.deepEqual(snapshot.state.resources[`pull:${mergedPullFiles.pull.number}:files`].map(file => file.path), mergedPullFiles.commitFiles.body.files.map(file => file.filename));
  ...
  assert.ok(endpoints.includes(commitEndpoint));
  ...
  const commitReads = endpoints.filter(endpoint => endpoint === commitEndpoint).length;
  await assert.rejects(
  ...
  assert.equal(endpoints.filter(endpoint => endpoint === commitEndpoint).length, commitReads);
});

test("merged commit file snapshots fail closed at the explicit 100-file bound", async () => {
  const files = Array.from({ length: 100 }, (_, index) => ({
    filename: `smith/src/file-${index}.rs`, status: "modified", additions: 1, deletions: 0, patch: "@@",
  }));
  const { github, endpoints, commitEndpoint } = mergedRoleGitHub(files);
  await assert.rejects(
    () => github.readRoleSnapshot(mergedObligationEvent(), role("docs-writer"), { controlSha: "a".repeat(40), appId: appIdentity.appId }),
    error => error?.code === "forge" && error.message === "overflow",
  );
  assert.ok(endpoints.includes(commitEndpoint));
  assert.equal(endpoints.some(endpoint => endpoint.startsWith(`/repos/bugabinga/smith/pulls/${mergedPullFiles.pull.number}/files?`)), false);
...
test("auditor reads App-omitted merge settings through one exact bounded GraphQL query", async () => {
  ...
  const github = adapter(async request => {
    ...
    if (endpoint.startsWith(`/repos/bugabinga/smith/pulls/${mergedPullFiles.pull.number}/files?`)) throw new AdwError("provider", "exit", { httpStatus: mergedPullFiles.pullFiles.status });
    if (endpoint.startsWith(`/repos/bugabinga/smith/commits/${mergedPullFiles.pull.merge_commit_sha}`)) return reply(mergedPullFiles.commitFiles.body);
...
test("reconciler derives merged obligations and closing authority from merge-commit paths", async () => {
  const endpoints = [];
  const mergeSha = mergedPullFiles.pull.merge_commit_sha;
  const commitEndpoint = `/repos/bugabinga/smith/commits/${mergeSha}?per_page=100&page=1`;
  const github = adapter(async request => {
    ...
    if (endpoint.startsWith("/repos/bugabinga/smith/pulls?state=all")) return reply([mergedPullFiles.pull]);
    if (endpoint.startsWith(`/repos/bugabinga/smith/pulls/${mergedPullFiles.pull.number}/files?`)) throw new AdwError("provider", "exit", { httpStatus: mergedPullFiles.pullFiles.status });
    if (endpoint === commitEndpoint) return reply(mergedPullFiles.commitFiles.body);
    if (endpoint.startsWith("/repos/bugabinga/smith/labels?")) return reply([]);
  ...
  const pull = snapshot.state.resources.pulls[0];
  assert.deepEqual(pull.changedPaths, mergedPullFiles.commitFiles.body.files.map(file => file.filename).sort());
