...
test("reconciler derives merged obligations with explicitly unavailable paths and no file APIs", async () => {
  ...
  const github = adapter(async request => {
    ...
    if (/\/repos\/bugabinga\/smith\/actions\/workflows\/adw-(?:issues|pulls|operations)\.yml\/runs\?(?:status=completed|event=repository_dispatch)&per_page=100&page=1$/.test(endpoint)) return reply({ workflow_runs: [] });
    if (endpoint.startsWith(`/repos/bugabinga/smith/issues/${mergedPullFiles.pull.number}/comments?`)) return reply([]);
    throw new Error(`unexpected ${endpoint}`);
  });
...
test("live-30713540804-shaped recovery binds the exact run and failed/cancelled jobs", async () => {
  ...
  const endpoints = [];
  let completedPullRuns = [fixture.cancelledPullRun];
  const github = adapter(async request => {
    const endpoint = request.args.at(-1);
    endpoints.push(endpoint);
    const reply = value => ({ code: 0, signal: null, stdout: JSON.stringify(value), stderr: "" });
    ...
    if (endpoint === "/repos/bugabinga/smith/actions/workflows/adw-pulls.yml/runs?status=completed&per_page=100&page=1") return reply({ workflow_runs: completedPullRuns });
    if (endpoint === "/repos/bugabinga/smith/actions/workflows/adw-operations.yml/runs?status=completed&per_page=100&page=1") return reply({ workflow_runs: [fixture.failedPreGraphRun] });
    ...
    if (/\/actions\/workflows\/adw-(?:pulls|operations)\.yml\/runs\?event=repository_dispatch&per_page=100&page=1$/.test(endpoint)) return reply({ workflow_runs: [] });
  ...
  assert.deepEqual(reruns.map(operation => operation.failedJobs.length), [5]);
  ...
  assert.ok(endpoints.includes("/repos/bugabinga/smith/actions/workflows/adw-operations.yml/runs?status=completed&per_page=100&page=1"));
  assert.equal(endpoints.some(endpoint => endpoint.includes("actions/workflows/adw-maintenance.yml")), false);
  ...
  assert.equal(snapshot.state.resources.runs.some(run => run.id === "30713540804" && run.name === "ADW pull and reconcile triggers" && run.displayTitle === "ADW pull #167" && run.controlSha === liveControlSha && run.applyJobId === "91405160611" && run.failedJobs.length === 5), true);
  ...
  assert.equal(fixture.failedPreGraphRun.name, ".github/workflows/adw-maintenance.yml");
  assert.equal(endpoints.some(endpoint => endpoint.includes(`/actions/runs/${fixture.failedPreGraphRun.id}/jobs?`)), false);

  const nonRegistryRuns = [
    { name: "Wrong workflow" },
    { path: ".github/workflows/adw-issues.yml" },
    { event: "issues" },
  ];
  for (const change of nonRegistryRuns) {
    completedPullRuns = [{ ...fixture.cancelledPullRun, ...change }];
    const ignoredSnapshot = await github.readControlSnapshot(event, "reconciler", { controlSha: liveControlSha, appId: appIdentity.appId });
    assert.deepEqual(ignoredSnapshot.state.reconciliation.cancelledApplies, []);
  }

  const malformedCandidateRuns = [
    [{ display_title: "ADW issue #167" }, "operational run entity is malformed"],
    [{ pull_requests: [{ ...fixture.cancelledPullRun.pull_requests[0], number: 168 }] }, "operational pull run entity is malformed"],
  ];
  for (const [change, message] of malformedCandidateRuns) {
    completedPullRuns = [{ ...fixture.cancelledPullRun, ...change }];
    await assert.rejects(
      () => github.readControlSnapshot(event, "reconciler", { controlSha: liveControlSha, appId: appIdentity.appId }),
      error => error?.code === "contract" && error.message === message,
    );
  }

  const oldControlRun = structuredClone(fixture.cancelledPullRun);
  oldControlRun.pull_requests[0].base.sha = "8".repeat(40);
  completedPullRuns = [oldControlRun];
  const oldControlSnapshot = await github.readControlSnapshot(event, "reconciler", { controlSha: liveControlSha, appId: appIdentity.appId });
  assert.deepEqual(oldControlSnapshot.state.reconciliation.cancelledApplies, []);
  assert.equal(oldControlSnapshot.state.resources.runs.some(run => run.id === String(oldControlRun.id)), false);
});
