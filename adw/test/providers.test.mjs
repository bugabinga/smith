...
test("Claude invocation receives only Claude credential and stamps envelope", async t => {
  ...
  const policy = productionRole("reviewer");
  t.after(() => rm(root, { recursive: true, force: true }));
  let call;
  ...
  const result = await invokeProvider({
    provider: "claude", executable: process.execPath, cliVersion: "2.1.220", rolePolicy: policy, snapshot,
  ...
  const outputSchema = JSON.parse(call.args[call.args.indexOf("--json-schema") + 1]);
  assert.equal(Object.hasOwn(outputSchema, "$schema"), false);
  assert.deepEqual(outputSchema, {
    type: "object", additionalProperties: false, required: ["outcome", "payload", "patch"],
    properties: {
      outcome: { enum: policy.payload.outcomes }, payload: JSON.parse(reviewSchema),
      patch: { anyOf: [
        { type: "null" },
        {
