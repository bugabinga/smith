...
test("Phase 5 records both rolled-back cutovers and blocks completion claims", async () => {
  const plan = await readFile(phase5Plan, "utf8");
  ...
  assert.match(plan, /third retry.*new protected PR.*fresh current-main base.*parent-correct.*rollback/is);
