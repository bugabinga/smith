...
test("renamed operational wrappers use only the independent MJS cutover barrier", async () => {
  ...
  assert.match(plan, /ninth retry.*remains required/i);
...
test("Phase 5 plan preserves pre-hold history and records the eighth signed rollback", async () => {
  const plan = await readFile(phase5Plan, "utf8");
  ...
  assert.match(plan, /Current `origin\/main` is signed eighth rollback `3e91aac769c8010a50f58c0a0c75e3aa85d3f817`/);
...
test("Phase 5 records all eight rolled-back cutovers and blocks completion claims", async () => {
  const plan = await readFile(phase5Plan, "utf8");
  ...
  assert.match(plan, /ninth retry.*required/i);
  assert.match(plan, /Phase 5 remains incomplete/i);
  assert.doesNotMatch(plan, /Expected: Phase 5 complete/);
});

test("Phase 5 preserves prior history and binds ninth-attempt commands", async () => {
  const plan = await readFile(phase5Plan, "utf8");
  ...
  assert.match(task12, /30859128166.*zero jobs\/artifacts.*3e91aac/is);
