...
test("Phase 5 plan preserves pre-hold history and records the seventh signed rollback", async () => {
  ...
  assert.match(plan, /Current `origin\/main` is signed seventh rollback `db80fd48c1c5fb7bd5a076f1c6b0571c3586361c`/);
...
test("Phase 5 records all seven rolled-back cutovers and blocks completion claims", async () => {
  const plan = await readFile(phase5Plan, "utf8");
  ...
  assert.match(plan, /eighth retry remains unexecuted/i);
...
test("Phase 5 preserves prior history and binds eighth-attempt commands", async () => {
  ...
  assert.match(task12, /30854346376.*queued.*zero jobs\/artifacts.*db80fd48/is);
