...
const names = ["adw-issues.yml", "adw-pulls.yml", "adw-operations.yml"];
...
const artifactSimulation = new URL("target-artifact-ref.test.sh", import.meta.url);
const executeFile = promisify(execFile);
const transportRefCommand = `/usr/bin/git -c core.hooksPath=/dev/null -c credential.helper= -c protocol.file.allow=never -c core.fsmonitor=false -C "\${{ github.workspace }}/target" update-ref --no-deref refs/heads/adw-target "$ADW_TARGET_SHA" && test "$(/usr/bin/git -c core.hooksPath=/dev/null -c credential.helper= -c protocol.file.allow=never -c core.fsmonitor=false -C "\${{ github.workspace }}/target" rev-parse --verify 'refs/heads/adw-target^{commit}')" = "$ADW_TARGET_SHA"`;
...
test("active charters enforce the assessment-only boundary and exclude release-manager", async () => {
  ...
  const docsCharter = await readFile(new URL("../../.claude/agents/docs-writer.md", import.meta.url), "utf8");
  assert.match(docsCharter, /changedPathsAvailability/);
  assert.match(docsCharter, /empty `changedPaths` does not prove that no files changed/);
  assert.match(docsCharter, /return `noop`[^.]*unavailable/i);
...
test("renamed operational wrappers use only the independent MJS cutover barrier", async () => {
  const values = await sources();
  for (const [file, source] of Object.entries(values)) {
    for (const [name, block] of jobs(source)) assert.match(block, /^    if: .*vars\.ADW_MJS_CUTOVER_HOLD != 'true'/m, `${file}:${name} lacks the MJS hold`);
    assert.doesNotMatch(source, /vars\.ADW_CUTOVER_HOLD/);
  }
  await assert.rejects(access(new URL("adw-maintenance.yml", wrapperDirectory)), error => error?.code === "ENOENT");
  await assert.rejects(access(new URL("adw-maintenance.yml", productionDirectory)), error => error?.code === "ENOENT");
  const plan = await readFile(phase5Plan, "utf8");
  assert.match(plan, /corrected retry.*remains required/i);
  assert.match(plan, /ADW_CUTOVER_HOLD=true.*permanently/is);
  assert.doesNotMatch(plan, /gh variable delete ADW_CUTOVER_HOLD/);
});
...
test("wrappers expose only exact triggers and semantically named lanes", async () => {
  ...
  const maintenanceOn = indentedBlock(values["adw-operations.yml"], "on");
  ...
  assert.match(values["adw-operations.yml"], /github\.event\.schedule == '57 2 \* \* \*' && 'alert-triager'/);
  assert.equal((values["adw-operations.yml"].match(/'alert-triager'/g) ?? []).length, 1);
  assert.doesNotMatch(values["adw-operations.yml"], /endsWith\(github\.event_name, '_alert'\)/);
  ...
  assert.ok(jobs(values["adw-operations.yml"]).has("maintenance-event-lane"));
  assert.match(values["adw-pulls.yml"], /pull-reconcile/);
  assert.match(values["adw-operations.yml"], /maintenance-(?:reconcile|audit)/);
  ...
  assert.match(values["adw-operations.yml"], /^run-name: "\$\{\{ github\.event_name == 'repository_dispatch' && github\.event\.client_payload\.smith_operation_digest \|\|.*\}\}"$/m);
  assert.doesNotMatch(Object.values(values).join("\n"), /inputs\.smith_operation_digest/);
  assert.match(values["adw-operations.yml"], /cron:\s*'7 \*\/6 \* \* \*'/);
...
test("wrapper event truth table prevents provider loops and reserves manual dispatch for owner controls", async () => {
  ...
  const maintenance = values["adw-operations.yml"];
...
test("top-level callers bind secret-bearing events to deployed default-branch bytes", async () => {
  ...
  const maintenanceLane = jobs(values["adw-operations.yml"]).get("maintenance-event-lane");
  const maintenanceCondition = inputLine(maintenanceLane, "if");
  assert.match(maintenanceCondition, /vars\.ADW_MJS_CUTOVER_HOLD != 'true' && github\.ref == 'refs\/heads\/main' && github\.workflow_sha == github\.sha/);
...
test("scheduled maintenance run names contain the exact cron expression", async () => {
  const maintenance = (await sources())["adw-operations.yml"];
...
test("trusted workflow SHA controls orchestration while entity head remains the target", async () => {
  ...
  for (const name of ["adw-pulls.yml", "adw-operations.yml"]) {
...
test("Phase 5 plan preserves pre-hold history and records the signed rollback chain", async () => {
  const plan = await readFile(phase5Plan, "utf8");
  assert.match(plan, /`ADW_CUTOVER_HOLD=true` permanently contains orphaned workflows/);
  assert.match(plan, /`ADW_MJS_CUTOVER_HOLD`.*armed before the first branch push/);
  ...
  assert.match(plan, /Current `origin\/main` is signed tenth rollback `1c23f50/);
  assert.match(plan, /`ADW_CUTOVER_HOLD=true` remains permanently armed.*legacy writers are active except.*`adw-release`/is);
});
...
test("Phase 5 records all ten rolled-back cutovers and blocks completion claims", async () => {
  const plan = await readFile(phase5Plan, "utf8");
  for (const evidence of [
    "#167", "2a31a5a", "d4dd6c6", "30746446905", "30747070695", "30747221797", "30747419140", "30766618499",
    "#168", "4cd7808", "2f3d9c9", "30768604275", "30769482565", "30771214058", "#169",
    "#170", "7ec4fbf", "58ff44b", "30775063472", "30775537057", "HTTP 422",
    "#171", "e91e4a1", "64a4515", "30787720045", "30788000713", "operational pull run entity is malformed",
    "#172", "2917520", "6c9a656", "30798105635", "prior apply result entry is invalid",
    "#173", "57361a6", "6538fd7", "30849959379", "30850408903", "30850775894",
    "#175", "737402d", "db80fd4", "30854346376", "325210492",
    "#176", "91aff79", "3e91aac", "30859128166", "30859241539",
    "#177", "7497f61", "9a5d2ca", "30884816241", "30884990977", "30885142692",
    "30886814052", "30887204135", "30887382006", "30888153701", "30888436716", "30889239515",
    "#178", "09d8626", "1c23f50", "30904565314", "30904705508", "30904879401",
  ...
  assert.match(plan, /temporary.*p38-adw-issue-write-probe\.yml.*live-apply-record-probe\.mjs.*deleted/is);
  assert.match(plan, /auditor snapshot intentionally used `mergedDetails=false`.*recomputation.*defaulted to `true`/is);
  ...
  assert.match(plan, /fourth attempt.*PR #171.*complete audit receipt.*signed.*rollback/is);
  assert.match(plan, /operation 0 `report_drift` completed.*operation 1 `sync_labels` failed/is);
  assert.match(plan, /fifth attempt.*operation 0 `report_drift`.*operation 1.*PR #163.*`publish_check`.*operation 2.*failed/is);
  ...
  assert.match(plan, /no GitHub write occurred/i);
  assert.match(plan, /no intent, write, or receipt/i);
  assert.match(plan, /operation 0 delivery completed.*globally named `runs` projection.*operation 1 correctly failed stale/is);
  assert.match(plan, /attempt 2.*flat prior-receipt parser.*restoring operation 0.*failed stale again/is);
  assert.match(plan, /seventh attempt.*no observed control defect.*forge scheduling failed safely/is);
  assert.match(plan, /normal cancellation and force cancellation.*HTTP 500.*DELETE.*HTTP 403/is);
  assert.match(plan, /corrected retry.*required/i);
  assert.match(plan, /Phase 5 remains incomplete/i);
  assert.doesNotMatch(plan, /Expected: Phase 5 complete/);
});

test("Phase 5 preserves prior history and binds the eleventh retry to signed tenth rollback", async () => {
  const plan = await readFile(phase5Plan, "utf8");
  ...
  for (const probe of [
    "../../.github/workflows/p38-target-artifact-probe.yml",
    "../../.github/workflows/p38-claude-cli-help-probe.yml",
    "../../.github/workflows/p38-claude-live-classify-probe.yml",
    "./live-claude-classify-probe.mjs",
  ]) await assert.rejects(access(new URL(probe, import.meta.url)), error => error?.code === "ENOENT");
  const task12 = plan.slice(plan.indexOf("### Task 12:"), plan.indexOf("### Task 13:"));
  assert.match(task12, /30904565314.*30904705508.*30904879401.*1c23f50/is);
...
test("Phase 5 binds the repaired urgent description and disposable probe outcome exactly", async () => {
  const plan = await readFile(phase5Plan, "utf8");
  const description = "Time-critical work; planner ranks it before equal- or lower-priority backlog items.";
  const probe = new URL("../../.github/workflows/p38-adw-label-write-probe.yml", import.meta.url);
  assert.equal([...description].length, 83);
  assert.match(plan, new RegExp(description.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
  assert.match(plan, /30775537057.*HTTP 422.*diagnosis evidence/is);
  assert.match(plan, /30776357596.*succeeded.*live `urgent` post-state.*`e03131`.*exact 83-character description/is);
  assert.match(plan, /prerequisite evidence for a fourth retry only.*does not establish deployment/is);
  assert.match(plan, /p38-adw-label-write-probe\.yml.*permanently deleted/is);
  await assert.rejects(access(probe), error => error?.code === "ENOENT");
  assert.doesNotMatch(plan, /Time-critical \(regression, security-adjacent/);
});
...
test("Phase 5 records live run-name recovery correction", async () => {
  const plan = await readFile(phase5Plan, "utf8");
  const task = plan.slice(plan.indexOf("### Task 7J:"), plan.indexOf("### Task 8:"));
  for (const phrase of [
    "30788000713", "operational pull run entity is malformed", "30713540804", "ADW pull and reconcile triggers", "ADW pull #167",
    "display_title", "closed operational workflow registry", "old-control run",
  ]) assert.match(task, new RegExp(phrase.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i"));
  assert.match(task, /never requires.*workflow name.*run title/is);
  assert.match(task, /focused GitHub.*32\/32.*full Node.*281\/281/is);
});

test("Phase 5 records the fifth rollback, immutable old-source probe failure, and flat artifact correction", async () => {
  const plan = await readFile(phase5Plan, "utf8");
  const task = plan.slice(plan.indexOf("### Task 7L:"), plan.indexOf("### Task 8:"));
  const probe = new URL("../../.github/workflows/p38-adw-apply-resume-probe.yml", import.meta.url);
  for (const phrase of [
    "30798105635", "attempt 1", "report_drift", "PR #163", "publish_check", "operation 2",
    "attempt 2", "prior apply result entry is invalid", "result.json", "result.sha256", "previous-apply-results",
    "6c9a656c66b38b8bf4771ad47a79fab8bd617bba", "30801002050",
    "2917520ee4842a8dbc9e57cce829a51710e5acff", "sixth-retry prerequisites",
  ]) assert.match(task, new RegExp(phrase.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i"));
  assert.match(task, /single pattern match.*directly.*not.*`adw-apply-result-1\/`/is);
  assert.match(task, /flat.*current attempt.*greater than one.*immediately previous attempt/is);
  assert.match(task, /mixed flat.*directories.*extra files.*symlinks.*missing sidecar.*attempt 1/is);
  assert.match(task, /multiple prior attempts.*directory layout.*latest/is);
  assert.match(task, /exact source artifact.*immutably bound.*old control.*old parser/is);
  assert.match(task, /not live evidence for the corrected parser.*live single-match shape.*full Node/is);
  assert.match(task, /no resumed operation or GitHub write/is);
  await assert.rejects(access(probe), error => error?.code === "ENOENT");
});

test("Phase 5 records one asynchronous reconciliation operation after sixth rollback", async () => {
  const plan = await readFile(phase5Plan, "utf8");
  const task = plan.slice(plan.indexOf("### Task 7M:"), plan.indexOf("### Task 8:"));
  for (const phrase of [
    "30849959379", "30850408903", "30850775894", "6538fd7cc4ad1a97c64a87539273c55469f51cd8",
    "canonical first dispatch", "exactly one", "rerun_check", "dispatch_repository", "add_label", "sync_labels",
    "globally named `runs` projection", "eighth retry",
  ]) assert.match(plan, new RegExp(phrase.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i"));
  assert.match(task, /strict RED.*multiple valid dispatch intents.*all five dispatch operations/is);
  assert.match(task, /canonical first `rerun_check`.*otherwise.*canonical first `dispatch_repository`/is);
  assert.match(task, /Global preconditions.*remain unchanged.*Workflow-run changes are not ignored.*not batched.*no queue or database/is);
});

test("Phase 5 records a green read prerequisite while requiring a fifth retry", async () => {
  const plan = await readFile(phase5Plan, "utf8");
  const task = plan.slice(plan.indexOf("### Task 7K:"), plan.indexOf("### Task 8:"));
  const workflow = new URL("../../.github/workflows/p38-adw-reconcile-read-probe.yml", import.meta.url);
  const script = new URL("./live-reconcile-read-probe.mjs", import.meta.url);
  for (const phrase of [
    "30790255777", "operational run identity is malformed", "30713498516", ".github/workflows/adw-maintenance.yml",
    "forge history candidates", "immutable current workflow registry", "valid cancellation control", "malformed candidate fields",
    "30793832261", "64a4515225f8f988989d38e21657bde97177202b",
  ]) assert.match(task, new RegExp(phrase.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i"));
  assert.match(task, /30793832261.*succeeded.*noncanonical historical workflow runs.*filtered/is);
  assert.match(task, /30790255777.*remains immutable evidence/is);
  assert.match(task, /fifth retry remains required/i);
  assert.match(task, /name.*path.*event.*return null.*never.*poison.*reconciliation/is);
  await assert.rejects(access(workflow), error => error?.code === "ENOENT");
  await assert.rejects(access(script), error => error?.code === "ENOENT");
});
...
test("cutover hold guards every operational job before tokens, artifacts, or assessment", async () => {
  ...
  for (const [file, source] of Object.entries(values)) for (const [name, block] of jobs(source)) {
    assert.match(block, /^    if: .*vars\.ADW_MJS_CUTOVER_HOLD != 'true'/m, `${file}:${name} lacks the global hold guard`);
  }
  const selftest = await readFile(new URL("../../.github/workflows/adw-selftest.yml", import.meta.url), "utf8");
  assert.doesNotMatch(selftest, /ADW_MJS_CUTOVER_HOLD/);
});
...
test("shared graph separates target data, prepare, providers, one fallback, decisions, verify, apply, and evidence", async () => {
  ...
  assert.match(graph.get("verify"), /if:.*vars\.ADW_MJS_CUTOVER_HOLD != 'true'.*always\(\)/);
  assert.match(graph.get("apply"), /if:.*vars\.ADW_MJS_CUTOVER_HOLD != 'true'.*always\(\)/);
  assert.match(graph.get("apply"), /group:\s*adw-write/);
  assert.match(graph.get("apply"), /cancel-in-progress:\s*false/);
  assert.match(graph.get("evidence"), /if:.*vars\.ADW_MJS_CUTOVER_HOLD != 'true'.*always\(\)/);
...
test("untrusted target checkout is isolated while control and target SHAs stay immutable", async () => {
  ...
  assert.doesNotMatch(target, /\bsecrets\.|ADW_PROVIDER_CREDENTIAL|create-github-app-token/);
  assert.deepEqual(runCommands(target), [transportRefCommand]);
  const checkout = target.indexOf("name: checkout immutable target");
  const bind = target.indexOf("name: bind exact transport ref");
  const upload = target.indexOf(`uses: ${ACTIONS.upload}`);
  assert.ok(checkout < bind && bind < upload, "transport ref must be bound after checkout and before upload");
  assert.match(target, /ADW_TARGET_SHA:\s*"\$\{\{ inputs\.target_sha \|\| github\.sha \}\}"/);
  assert.match(target, /GIT_CONFIG_NOSYSTEM:\s*"1".*GIT_CONFIG_GLOBAL:\s*\/dev\/null.*GIT_TERMINAL_PROMPT:\s*"0".*GIT_NO_REPLACE_OBJECTS:\s*"1"/s);
  assert.equal((target.match(/\/usr\/bin\/git/g) ?? []).length, 2);
  for (const setting of ["core.hooksPath=/dev/null", "credential.helper=", "protocol.file.allow=never", "core.fsmonitor=false"]) {
    assert.equal((target.match(new RegExp(setting.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g")) ?? []).length, 2, setting);
  }
  assert.match(target, /update-ref --no-deref refs\/heads\/adw-target "\$ADW_TARGET_SHA"/);
  assert.match(target, /rev-parse --verify 'refs\/heads\/adw-target\^\{commit\}'/);
...
test("actions, commands, App inputs, and policy-free YAML are exact", async () => {
  ...
  for (const name of ["adw-pulls.yml", "adw-operations.yml"]) assert.match(values[name], /APP_ID:\s*\$\{\{ secrets\.APP_ID \}\}/);
  ...
  const executed = runCommands(combined);
  const nonTransport = executed.filter(command => command !== transportRefCommand).join("\n");
  assert.doesNotMatch(nonTransport, /claude\s+-p|codex\s+exec|\b(?:gh|git|jq|npm|cargo|rustup|make)\s+/);
  assert.doesNotMatch(nonTransport, /\b(?:test|build|install|exec|eval|source)\s+(?:\.\/|["'])/);
  assert.deepEqual(new Set(executed), commands);
});

test("packed-ref artifact simulation fails before transport ref and survives its archive roundtrip", async () => {
  const { stdout, stderr } = await executeFile("bash", [fileURLToPath(artifactSimulation)], { timeout: 30_000 });
  assert.equal(stderr, "");
  assert.equal(stdout, "packed-only artifact: not-a-repository\ntransport-ref artifact: verified\n");
});
