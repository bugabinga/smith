...
test("prior apply reader preserves multiple-attempt directories and selects the latest canonical result", async t => {
  ...
  assert.deepEqual((await readdir(receipts)).sort(), ["adw-apply-result-1", "adw-apply-result-2"]);
...
test("prior apply reader treats the live single-match flat layout as immediately previous only after attempt one", async t => {
  const root = await mkdtemp(join(tmpdir(), "smith-adw-prior-flat-"));
  t.after(() => rm(root, { recursive: true, force: true }));
  const receipts = join(root, "previous-apply-results");
  const value = { layout: "live-single-match" };
  await writeTransportArtifact("applyResult", receipts, value);
  assert.deepEqual(await readPreviousApplyResult(receipts, 2), value);
  assert.deepEqual(await readPreviousApplyResult(receipts, 7), value);
  await assert.rejects(() => readPreviousApplyResult(receipts, 1), error => error?.code === "input");
});

test("prior apply reader rejects mixed, extra, incomplete, and symlinked flat layouts", async t => {
  const root = await mkdtemp(join(tmpdir(), "smith-adw-prior-flat-invalid-"));
  t.after(() => rm(root, { recursive: true, force: true }));

  const mixed = join(root, "mixed");
  await writeTransportArtifact("applyResult", mixed, { attempt: 1 });
  await mkdir(join(mixed, "adw-apply-result-1"));
  await assert.rejects(() => readPreviousApplyResult(mixed, 2), error => error?.code === "input");

  const extra = join(root, "extra");
  await writeTransportArtifact("applyResult", extra, { attempt: 1 });
  await writeFile(join(extra, "unexpected"), "");
  await assert.rejects(() => readPreviousApplyResult(extra, 2), error => error?.code === "input");

  const incomplete = join(root, "incomplete");
  await mkdir(incomplete);
  await writeFile(join(incomplete, "result.json"), canonicalBytes({ attempt: 1 }));
  await assert.rejects(() => readPreviousApplyResult(incomplete, 2), error => error?.code === "input");

  const source = join(root, "source");
  await writeTransportArtifact("applyResult", source, { attempt: 1 });
  const linked = join(root, "linked");
  await mkdir(linked);
  await symlink(join(source, "result.json"), join(linked, "result.json"));
  await writeFile(join(linked, "result.sha256"), await readFile(join(source, "result.sha256")));
  await assert.rejects(() => readPreviousApplyResult(linked, 2), error => error?.code === "input");
});
