...
test("sync_labels accepts 100 Unicode description characters and rejects 101 before mutation", async () => {
  const run = async count => {
    const definitions = [{ name: "limit", color: "abcdef", description: "😀".repeat(count) }];
    const source = JSON.stringify(definitions);
    const trusted = { trust: "trusted", source: ".github/labels.yml", bytes: canonicalBytes(source).length, digest: digestJson(source), data: source };
    const value = snapshot([
      { resource: "labels", kind: "labels", token: digestJson([]) },
    ...
    const { github, calls } = harness(request => {
      ...
      if (request.args[2] === "GET" && path.includes("/labels?")) return reply([...live.values()]);
      if (request.args[2] === "GET" && path.includes("/labels/")) return reply(live.get(decodeURIComponent(path.split("/").at(-1))));
      if (request.args[2] === "POST") { const label = { id: 1, ...body(request) }; live.set(label.name, label); return reply(label); }
      throw new Error(`unexpected ${request.args[2]} ${path}`);
    });
    return { github, calls, definitions, value };
  };

  const accepted = await run(100);
  assert.equal((await apply(accepted.github, { type: "sync_labels", definitionsDigest: digestJson(accepted.definitions) }, accepted.value)).status, "complete");
  assert.equal(accepted.calls.filter(call => call.args[2] === "POST").length, 1);

  const rejected = await run(101);
  await assert.rejects(() => apply(rejected.github, { type: "sync_labels", definitionsDigest: digestJson(rejected.definitions) }, rejected.value), error => error?.code === "contract");
  assert.equal(rejected.calls.filter(call => call.args[2] !== "GET").length, 0);
});
