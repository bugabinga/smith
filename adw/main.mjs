...
export async function readPreviousApplyResult(root, currentAttempt) {
  ...
  const flatNames = ["result.json", "result.sha256"];
  if (entries.some(entry => flatNames.includes(entry.name))) {
    const names = entries.map(entry => entry.name).sort();
    if (currentAttempt === 1 || names.length !== flatNames.length || names.some((name, index) => name !== flatNames[index])) inputError("prior apply result entry is invalid");
    const result = (await readTransportArtifact("applyResult", root)).value;
    await assertDirectoryIdentity(claim);
    return result;
  }
  const candidates = [];
