...
The tenth attempt for PR #178 produced complete audit and reconciliation receipts, but its dispatched docs-writer failed safely during target preparation and the attempt ended in signed rollback:

- Signed aggregate `28f84ba7055b437fb95fb4a5462da89b6d6d3e6b` and signed planning head `1266869b7917b1043b0a889407a6a8ac494fb77d` landed as tenth cutover squash `09d862682763b1b15f6f475a8fa7d0a14c54cbdb` over signed ninth rollback `9a5d2caeefbf1cd23ec7ee90045a9655676cf8bd`.
- Owner-dispatched audit run [`30904565314`](https://github.com/bugabinga/smith/actions/runs/30904565314) succeeded with a complete receipt. Reconciliation run [`30904705508`](https://github.com/bugabinga/smith/actions/runs/30904705508) also succeeded with one complete `dispatch_repository` operation and receipt.
...
The rolled-back tenth attempt used owner-authored PR #178 from `adw/mjs-phase5-retry9`, based on signed ninth rollback `9a5d2caeefbf1cd23ec7ee90045a9655676cf8bd`. Signed aggregate `28f84ba7055b437fb95fb4a5462da89b6d6d3e6b`, signed planning head `1266869b7917b1043b0a889407a6a8ac494fb77d`, cutover `09d862682763b1b15f6f475a8fa7d0a14c54cbdb`, complete audit `30904565314`, complete reconciliation dispatch receipt `30904705508`, pre-provider docs-writer failure `30904879401`, and signed rollback `1c23f5000ecdf049b31f089527dd4c2e557bf582` are historical evidence. Root cause: artifact transport drops the checkout's empty `.git/refs`, leaving packed refs without the directory Git requires for repository discovery.
...
### Task 7N: Preserve target repository identity across artifact transport

**Files:**

- Modify: `.github/workflows/adw-issues.yml`
- Modify: `prototypes/p38-adw-disposable/wrappers/adw-issues.yml`
- Create: `adw/test/target-artifact-ref.test.sh`
- Modify: `adw/test/wrappers.test.mjs`
- Modify: `docs/super/plans/2026-08-01-adw-mjs-control-plane-phase-5-production-cutover-proof.md`
- Delete: `.github/workflows/p38-target-artifact-probe.yml`

- [x] **Step 1: Materialize and bind one explicit transport ref**

Target preparation creates `refs/heads/adw-target` with hardened credential-free Git before upload and immediately verifies that it resolves to exact `ADW_TARGET_SHA`. The provider-side prepare job verifies that same full ref after download before any provider credential is available. This preserves a nonempty `.git/refs` hierarchy without changing detached `HEAD`, accepting an ambiguous revision, fetching, or adding transport credentials. Production and disposable wrappers remain byte-identical.

- [x] **Step 2: Simulate the artifact roundtrip locally**

`adw/test/target-artifact-ref.test.sh` creates a detached checkout whose refs are packed, binds only explicit `refs/heads/adw-target`, copies it through a hidden-file-preserving artifact simulation, and proves the transported ref resolves to the exact target commit. Wrapper contracts bind the hardened `update-ref --no-deref` command, immediate exact-SHA check, upload ordering, and post-download exact-SHA verification.
...
for wrapper in adw-issues.yml adw-operations.yml adw-pulls.yml; do
...
gh pr view "$PR" --repo bugabinga/smith \
  ...
  jq -e --argjson pr "$PR" --arg head "$PR_HEAD" --arg branch "$BRANCH" '
    .number == $pr and .state == "OPEN" and .headRefName == $branch and
...
Expected: the owner-created PR remains on its exact owner-approved signed aggregate and main remains exactly signed tenth rollback `1c23f5000ecdf049b31f089527dd4c2e557bf582`.
...
CUTOVER_BASE=1c23f5000ecdf049b31f089527dd4c2e557bf582
...
Expected: both patches derive only from signed rollback base `1c23f50` and the exact approved eleventh-attempt head; the reverse patch restores the rollback tree and contains no secret values.
...
git branch -f fix/rollback-adw-mjs-phase5-retry10-rehearsal "$REHEARSAL_SHA"
git bundle create "$ROLLBACK_ROOT/rehearsal.bundle" \
  refs/heads/fix/rollback-adw-mjs-phase5-retry10-rehearsal
...
Legacy writers are currently active except `adw-release`; that rollback state is not an eleventh-attempt drain. Disable and observe all 21 paths again inside the owner-created PR's quiet window.
...
jq -e --argjson id "$OWNER_APPROVAL_COMMENT_ID" \
  ...
  --arg issue "https://api.github.com/repos/bugabinga/smith/issues/$PR" '
    .id == $id and
...
test -z "$(git ls-remote --heads origin refs/heads/fix/rollback-adw-mjs-phase5-retry10)"
git branch -f fix/rollback-adw-mjs-phase5-retry10 "$ROLLBACK_SHA"
git push origin refs/heads/fix/rollback-adw-mjs-phase5-retry10
git bundle create "$ROLLBACK_ROOT/parent-correct.bundle" \
  refs/heads/fix/rollback-adw-mjs-phase5-retry10
...
Expected: prepared signed rollback is a direct child of the eleventh cutover squash, restores exact base `1c23f50`, and is available at `fix/rollback-adw-mjs-phase5-retry10` plus the private bundle. All ten historical rollback branches and commits remain untouched. Every eleventh-attempt run created under `ADW_MJS_CUTOVER_HOLD` must be `completed`/`skipped`, zero-artifact, and either zero-job or all-skipped before that independent hold is deleted. `ADW_CUTOVER_HOLD=true` remains permanent; queued old-path runs `30854346376` and `30859128166` are never cancellation or release blockers for the fresh identity.
...
**Historical fourth- through tenth-attempt execution:** Fourth-attempt audit `30787720045` succeeded with a complete receipt; fifth and sixth attempts retained the partial/success evidence recorded above. Seventh-attempt run `30854346376` and eighth-attempt run `30859128166` never started a job and remain immutable queue evidence. Ninth audit `30884816241` and reconciliation `30884990977` completed, but child `30885142692` exited before artifact creation and rollback `9a5d2caeefbf1cd23ec7ee90045a9655676cf8bd` restored legacy authority. Tenth audit `30904565314` and reconciliation `30904705508` succeeded with one complete dispatch receipt, but docs-writer child `30904879401` failed prepare at `verification:git` before provider execution; signed rollback `1c23f5000ecdf049b31f089527dd4c2e557bf582` restored legacy authority. Those runs do not complete the eleventh retry; the unchecked commands below are its future procedure.
...
> **Historical fourth-attempt blocker:** Run `30788000713` failed in prepare before any artifact upload with `operational pull run entity is malformed`. No child dispatch, apply, write, or receipt occurred, and rollback followed; this does not complete the eleventh-attempt step.
...
Owner comments acceptance on the evidence. Normal main merges may resume. Keep `fix/rollback-adw-mjs-phase5-retry10` until Phase 6 explicitly retires it; retain historical rollback refs through `fix/rollback-adw-mjs-phase5-retry9` as first- through tenth-attempt evidence.

Expected after a fully proved eleventh cutover: owner acceptance may complete Phase 5. Earlier audit, receipt, stale-rejection, queue-failure, rollback, diagnostic, and fresh-path evidence remains historical. The eleventh attempt remains unexecuted; Phase 6 remains separate.
...
- any eleventh-attempt run created under `ADW_MJS_CUTOVER_HOLD` not reaching `completed`/`skipped` with zero artifacts and zero jobs or only skipped jobs before that independent hold is released;
...
ROLLBACK_ROOT="$HOME/.local/state/smith-adw-phase5-retry10-rollback"
EXPECTED_CUTOVER_BASE=1c23f5000ecdf049b31f089527dd4c2e557bf582
...
PREPARED_ROLLBACK_SHA=$(git rev-parse origin/fix/rollback-adw-mjs-phase5-retry10)
