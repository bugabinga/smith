...
- The owner keeps main otherwise quiet from the final head check until both scheduled cycles finish. For any further retry, the strict `main == MERGE_SHA` quiet-main invariant starts only when its new protected PR lands. The PR #167, PR #168, PR #170, and PR #171 cutovers and rollbacks are historical evidence and do not satisfy or start another retry interval. Any subsequent unexpected main movement invalidates current-head evidence, stops proof, and triggers rollback from the new current main; it never authorizes force-push or reset.
...
**Current prerequisite and probe status:** Phase 5 remains incomplete and all four cutovers are rolled back. Earlier disposable probes, temporary `.github/workflows/p38-adw-issue-write-probe.yml`, `adw/test/live-apply-record-probe.mjs`, and temporary `.github/workflows/p38-adw-label-write-probe.yml` remain deleted under promote-or-delete. Fourth-attempt audit `30787720045` is a complete positive receipt, while reconciliation `30788000713` and probe `30790255777` remain preserved pre-artifact failure evidence. Exact read probe `30793832261` is green prerequisite evidence only. Temporary `.github/workflows/p38-adw-reconcile-read-probe.yml` and `adw/test/live-reconcile-read-probe.mjs` are permanently deleted under promote-or-delete. A fifth retry remains required; no scheduled-cycle proof or Phase 5 completion exists.
...
Phase 5 remains incomplete after cutovers `2a31a5a`, `4cd7808`, `7ec4fbf`, and `e91e4a1` and signed rollbacks `d4dd6c6`, `2f3d9c9`, `58ff44b`, and `64a4515`. Fourth-attempt audit `30787720045` succeeded complete, but reconciliation `30788000713` failed before artifacts and signed fourth rollback `64a4515` restored legacy authority. Failed read probe `30790255777` remains preserved diagnosis evidence; exact read probe `30793832261` succeeded after noncanonical run filtering and closes only that retry prerequisite. A fifth retry remains required. Phase 5 can end only after a future GitHub-verified cutover leaves three MJS operational wrappers plus self-test as sole ADW authority, release automation deferred, a parent-correct signed rollback available, all required positive production receipts captured, explicit behind/dirty candidates still fail-closed, and two consecutive scheduled reconciliation cycles green. Prior failed audits, read-only and exact App probes, isolated operation run `30771214058`/#169, and the fourth complete audit remain retry evidence, not phase-completion evidence; legacy marker compatibility, long-term docs, rollback-branch retirement, and backlog cleanup remain Phase 6.
