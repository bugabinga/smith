...
- This is a forge queue/control-record failure, not evidence of an MJS control defect or positive audit execution. An eighth cutover must retain the hold until old run `30854346376` reaches status/conclusion `completed`/`skipped`, has zero artifacts, and has either zero jobs or only skipped jobs. If it remains queued, the hold must not be released and MJS must be rolled back again.

**Current prerequisite and probe status:** Phase 5 remains incomplete and all seven cutovers are rolled back. Earlier disposable probes, temporary `.github/workflows/p38-adw-issue-write-probe.yml`, `adw/test/live-apply-record-probe.mjs`, temporary `.github/workflows/p38-adw-label-write-probe.yml`, temporary `.github/workflows/p38-adw-reconcile-read-probe.yml`, `adw/test/live-reconcile-read-probe.mjs`, temporary `.github/workflows/p38-adw-apply-resume-probe.yml`, and all other temporary probe files remain deleted under promote-or-delete. Fourth-attempt audit `30787720045`, sixth-attempt audits `30849959379` and `30850408903`, fifth-attempt partial receipts, and sixth-attempt operation 0 are preserved historical evidence. Seventh-attempt run `30854346376` is immutable zero-job/zero-artifact queue evidence only. An eighth retry remains required; no scheduled-cycle proof and no Phase 5 completion exist.

## Current post-rollback baseline

Current `origin/main` is signed seventh rollback `db80fd48c1c5fb7bd5a076f1c6b0571c3586361c`, whose sole parent is seventh cutover `737402d1de1ce57cbdcd8553c3a9829f91be4847` and whose tree exactly equals signed sixth rollback `6538fd7cc4ad1a97c64a87539273c55469f51cd8`, restoring the legacy baseline. `ADW_CUTOVER_HOLD=true` remains armed, and legacy writers are active except deferred `adw-release`, which remains disabled. The prior retry used owner-authored PR #168 from `adw/mjs-phase5-retry`, based on first rollback `d4dd6c6d0e828c876aab5470e8e80524b6cd7e84`, and landed as cutover `4cd78089db44306f8a7f9c6855accff0ae5c8c29`; signed aggregate commit `f79462666647a0c504479f68fe4083050d9e5f9d` remains historical implementation provenance, not current deployment evidence.
...
OWNER_APPROVAL_MARKER="Owner approval: quiet-window MJS production cutover eighth attempt and positive-only proof on PR #$PR exact head $PR_HEAD."
...
printf 'pr=%s\nowner_approval_comment_id=%s\nowner_approval_created_at=%s\nowner_approval_marker=%s\n' \
  "$PR" "$OWNER_APPROVAL_COMMENT_ID" "$OWNER_APPROVAL_COMMENT_CREATED_AT" \
...
test "$(git rev-parse origin/main)" = "db80fd48c1c5fb7bd5a076f1c6b0571c3586361c"
```

Expected: forge-derived `PR` remains on its owner-approved aggregate head and main remains exactly signed rollback base `db80fd48c1c5fb7bd5a076f1c6b0571c3586361c`.

- [ ] **Step 4: Create an encrypted/private rollback workspace and reverse patch**

```bash
ROLLBACK_ROOT="$HOME/.local/state/smith-adw-phase5-retry7-rollback"
mkdir -p "$ROLLBACK_ROOT"
chmod 700 "$ROLLBACK_ROOT"
CUTOVER_BASE=db80fd48c1c5fb7bd5a076f1c6b0571c3586361c
...
Expected: both patches are derived only from signed rollback base `db80fd4` and the exact approved eighth-attempt head; the reverse patch restores the rollback tree and contains no secret values.
...
git branch -f fix/rollback-adw-mjs-phase5-retry7-rehearsal "$REHEARSAL_SHA"
git bundle create "$ROLLBACK_ROOT/rehearsal.bundle" \
  refs/heads/fix/rollback-adw-mjs-phase5-retry7-rehearsal
...
Legacy writers are currently active except `adw-release`; that rollback state is not an eighth-attempt drain. Disable and observe all 21 paths again inside the new PR's quiet window.
...
set -euo pipefail
test "$PR_HEAD" = "$(gh pr view "$PR" --repo bugabinga/smith --json headRefOid --jq .headRefOid)"
test "$CUTOVER_BASE" = db80fd48c1c5fb7bd5a076f1c6b0571c3586361c
...
jq -e --argjson id "$OWNER_APPROVAL_COMMENT_ID" \
  ...
  --arg head "$PR_HEAD" --argjson pr "$PR" '
    ...
    .user.id == 876467 and .user.login == "bugabinga" and
    .author_association == "OWNER" and .body == $body and
    .body == ("Owner approval: quiet-window MJS production cutover eighth attempt and positive-only proof on PR #" + ($pr | tostring) + " exact head " + $head + ".") and
...
test -z "$(git ls-remote --heads origin refs/heads/fix/rollback-adw-mjs-phase5-retry7)"
git branch -f fix/rollback-adw-mjs-phase5-retry7 "$ROLLBACK_SHA"
git push origin refs/heads/fix/rollback-adw-mjs-phase5-retry7
git bundle create "$ROLLBACK_ROOT/parent-correct.bundle" \
  refs/heads/fix/rollback-adw-mjs-phase5-retry7
...
Expected: prepared signed rollback is a direct child of the eighth cutover squash, restores exact base `db80fd4`, and is available at `fix/rollback-adw-mjs-phase5-retry7` plus the private bundle. All seven historical rollback branches and commits remain untouched. Old run `30854346376` and every eighth-attempt held MJS run must be `completed`/`skipped`, zero-artifact, and either zero-job or all-skipped before hold deletion. If the old run remains queued, this step exits with the hold retained and triggers rollback; never release the hold to continue proof.
...
**Historical fourth- through seventh-attempt execution:** Fourth-attempt audit `30787720045` succeeded with a complete receipt; reconciliation `30788000713` then failed in prepare with zero artifacts and rollback `64a4515225f8f988989d38e21657bde97177202b` followed. Fifth-attempt audit retry `30798105635` retained a partial receipt before retry recovery failed and rollback `6c9a656c66b38b8bf4771ad47a79fab8bd617bba` followed. Sixth-attempt audits `30849959379` and `30850408903` succeeded; reconciliation `30850775894` completed dispatch operation 0, then correctly failed stale before rollback `6538fd7cc4ad1a97c64a87539273c55469f51cd8`. Seventh-attempt audit `30854346376` never started a job: it remained queued with zero jobs/artifacts, cancellation returned HTTP 500, DELETE returned HTTP 403, and rollback `db80fd48c1c5fb7bd5a076f1c6b0571c3586361c` followed under the retained hold. This evidence remains historical; the unchecked commands below are eighth-attempt commands and are not evidence of authorization execution or completion.
...
> **Historical fourth-attempt blocker:** Run `30788000713` failed in prepare before any artifact upload with `operational pull run entity is malformed`. No child dispatch, apply, write, or receipt occurred, and rollback followed; this does not complete the eighth-attempt step.
...
Owner comments acceptance on the evidence. Normal main merges may resume. Keep `fix/rollback-adw-mjs-phase5-retry7` until Phase 6 explicitly retires it; retain historical rollback refs through `fix/rollback-adw-mjs-phase5-retry6` as first- through seventh-attempt evidence.

Expected after a fully proved eighth cutover: owner acceptance may complete Phase 5. The fourth attempt completed cutover and audit, then failed reconciliation and rolled back. The fifth attempt reached partial audit writes, failed receipt recovery, and rolled back. The sixth attempt completed two audits and one dispatch delivery, then correctly failed stale after child-run progression and rolled back. The seventh attempt had no observed control defect, but its audit remained queued with zero jobs/artifacts and rollback followed. The eighth attempt remains unexecuted; Phase 6 remains separate.
...
- old held run `30854346376` not reaching `completed`/`skipped` with zero artifacts and zero jobs or only skipped jobs before hold release;
...
PR=${PRS[0]}
ROLLBACK_ROOT="$HOME/.local/state/smith-adw-phase5-retry7-rollback"
EXPECTED_CUTOVER_BASE=db80fd48c1c5fb7bd5a076f1c6b0571c3586361c
...
PREPARED_ROLLBACK_SHA=$(git rev-parse origin/fix/rollback-adw-mjs-phase5-retry7)
