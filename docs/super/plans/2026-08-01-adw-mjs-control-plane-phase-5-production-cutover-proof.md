...
- The owner keeps main otherwise quiet from the final head check until both scheduled cycles finish. For any further retry, the strict `main == MERGE_SHA` quiet-main invariant starts only when its new protected PR lands. The PR #167, PR #168, PR #170, PR #171, and PR #172 cutovers and rollbacks are historical evidence and do not satisfy or start another retry interval. Any subsequent unexpected main movement invalidates current-head evidence, stops proof, and triggers rollback from the new current main; it never authorizes force-push or reset.
...
**Current prerequisite and probe status:** Phase 5 remains incomplete and all five cutovers are rolled back. Earlier disposable probes, temporary `.github/workflows/p38-adw-issue-write-probe.yml`, `adw/test/live-apply-record-probe.mjs`, temporary `.github/workflows/p38-adw-label-write-probe.yml`, temporary `.github/workflows/p38-adw-reconcile-read-probe.yml`, `adw/test/live-reconcile-read-probe.mjs`, and temporary `.github/workflows/p38-adw-apply-resume-probe.yml` remain deleted under promote-or-delete. Fourth-attempt audit `30787720045` remains complete positive evidence; fifth-attempt audit retry `30798105635` remains partial write and failed recovery evidence. Failed apply-resume probe `30801002050` is immutable-transport diagnosis only; focused unit tests reproducing the live single-match shape and full Node remain corrected-parser retry-prerequisite evidence. Exact read probe `30793832261` remains green prerequisite evidence only. A sixth retry remains required; no reconciliation or scheduled-cycle proof and no Phase 5 completion exist.

## Current post-rollback baseline

Current `origin/main` is signed fifth rollback `6c9a656c66b38b8bf4771ad47a79fab8bd617bba`, whose sole parent is fifth cutover `2917520ee4842a8dbc9e57cce829a51710e5acff` and whose tree exactly equals signed fourth rollback `64a4515225f8f988989d38e21657bde97177202b`, restoring the legacy baseline. `ADW_CUTOVER_HOLD=true` remains armed, and legacy writers are active except deferred `adw-release`, which remains disabled. The prior retry used owner-authored PR #168 from `adw/mjs-phase5-retry`, based on first rollback `d4dd6c6d0e828c876aab5470e8e80524b6cd7e84`, and landed as cutover `4cd78089db44306f8a7f9c6855accff0ae5c8c29`; signed aggregate commit `f79462666647a0c504479f68fe4083050d9e5f9d` remains historical implementation provenance, not current deployment evidence.
...
The sixth candidate uses owner-authored PR #173 from `adw/mjs-phase5-retry5`, based on signed fifth rollback `6c9a656c66b38b8bf4771ad47a79fab8bd617bba`. Signed direct-child aggregate `60d471bab66f6bafc1774281b5bbc6d422250944` is its implementation anchor. The hold remains `true`, legacy authority remains active except disabled `adw-release`, and no sixth cutover, deployment, production proof, or Phase 5 completion has occurred.
...
### Task 8: Push the sixth-attempt plan and record owner approval on its exact checked head
...
**Sixth-attempt procedure:** PR #173 and branch `adw/mjs-phase5-retry5` currently name signed aggregate `60d471bab66f6bafc1774281b5bbc6d422250944` over signed rollback base `6c9a656c66b38b8bf4771ad47a79fab8bd617bba`. The unchecked commands below first update the PR head to this signed planning commit, then validate its current-head check, and only afterward create exact-head owner approval. They are future authorization gates, not evidence of deployment or Phase 5 completion.
...
Expected after a later fully proved cutover: owner acceptance may complete Phase 5. The fourth attempt completed cutover and audit, then failed reconciliation and rolled back. The fifth attempt completed cutover, reached partial audit writes, failed receipt recovery, and rolled back. A sixth retry remains required; Phase 6 remains separate.
...
Phase 5 remains incomplete after cutovers `2a31a5a`, `4cd7808`, `7ec4fbf`, `e91e4a1`, and `2917520` and signed rollbacks `d4dd6c6`, `2f3d9c9`, `58ff44b`, `64a4515`, and `6c9a656`. Fourth-attempt audit `30787720045` succeeded complete, but reconciliation `30788000713` failed before artifacts. Fifth-attempt audit retry `30798105635` retained two complete operation receipts, then attempt-2 flat receipt recovery failed before any apply operation; signed fifth rollback `6c9a656` restored legacy authority. Apply-resume probe `30801002050` failed because immutable old-control `2917520` source necessarily retained the old parser; it proves retroactive repair is impossible and is not evidence for the corrected parser. Failed read probe `30790255777` remains preserved diagnosis evidence; exact read probe `30793832261` succeeded after noncanonical run filtering. Focused live-shape unit tests and full Node remain retry prerequisites, and a sixth retry remains required. Phase 5 can end only after a future GitHub-verified cutover leaves three MJS operational wrappers plus self-test as sole ADW authority, release automation deferred, a parent-correct signed rollback available, all required positive production receipts captured, explicit behind/dirty candidates still fail-closed, and two consecutive scheduled reconciliation cycles green. Prior failed audits, read-only and exact App probes, isolated operation run `30771214058`/#169, the fourth complete audit, and fifth partial receipts remain retry evidence, not phase-completion evidence; legacy marker compatibility, long-term docs, rollback-branch retirement, and backlog cleanup remain Phase 6.
