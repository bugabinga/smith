...
**Current prerequisite and probe status:** Phase 5 remains incomplete and both cutovers are rolled back. Earlier disposable probes and the temporary `.github/workflows/p38-adw-issue-write-probe.yml` plus `adw/test/live-apply-record-probe.mjs` were deleted under promote-or-delete. Do not resume Tasks 8–16 from either attempt's stale values. A third retry requires a new protected PR, fresh current-main base and candidate head, fresh owner approval and quiet-window observations, and parent-correct signed rollback material.

## Current post-rollback baseline

Current `origin/main` is the signed second rollback `2f3d9c98b9f678139b0f8cd2e671afde16ffbc98`, whose parent is retry cutover `4cd78089db44306f8a7f9c6855accff0ae5c8c29` and whose tree restores the legacy baseline. The prior retry used owner-authored PR #168 from `adw/mjs-phase5-retry`, based on first rollback `d4dd6c6d0e828c876aab5470e8e80524b6cd7e84`; signed aggregate commit `f79462666647a0c504479f68fe4083050d9e5f9d` remains historical implementation provenance, not current deployment evidence.

The third attempt uses owner-authored PR #170 from `adw/mjs-phase5-retry2`, based on signed second rollback `2f3d9c98b9f678139b0f8cd2e671afde16ffbc98`; signed aggregate commit `273dda4cd2a9dc77fc2193498886a5badc389895` is its implementation anchor. `ADW_CUTOVER_HOLD=true` remains armed, and legacy writers remain active except deferred `adw-release`. Tasks 8–16 remain unchecked and are rebound below to this attempt. Every mutable forge fact must be re-read. Failed audit runs, read-only probes, and isolated issue #169 are immutable diagnosis only and do not satisfy deployment or positive-proof gates.
...
Phase 5 remains incomplete after cutovers `2a31a5a` and `4cd7808` and signed rollbacks `d4dd6c6` and `2f3d9c9`. It can end only after a third, GitHub-verified cutover leaves three MJS operational wrappers plus self-test as sole ADW authority, release automation deferred, a parent-correct signed rollback available, all required positive production receipts captured, explicit behind/dirty candidates still fail-closed, and two consecutive scheduled reconciliation cycles green. Failed audit `30768604275`, read-only probes, and isolated operation run `30771214058`/#169 are retry evidence, not phase-completion evidence; legacy marker compatibility, long-term docs, rollback-branch retirement, and backlog cleanup remain Phase 6.
