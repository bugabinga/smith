...
2. Otherwise, survey the gap: read `docs/SPEC.md` (the goal), the build order in
   ...
   closed.
3. Open **one** work-order issue for it, filed under the current milestone: one
   deliverable, its SPEC anchor, acceptance in the spec's own terms. Route it to a
   builder by surface, exactly as the triager does: a **UI/UX / TUI / frontend**
   slice gets `ready` (the Claude builder), a **backend / core / engine** slice
   gets `codex` (the Codex builder). That is enough — the builder takes it from there.
