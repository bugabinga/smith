...
run 'fallback:claude,codex' '[{"id":1,"user":{"login":"smith[bot]"},"body":"<!-- smith:builder-route/v1 issue=1 id=123e4567-e89b-12d3-a456-426614174000 source=claude/issue-1 target=codex/issue-1 phase=completed -->"}]'
grep -Eq 'issue edit 1 --repo owner/repo --remove-label codex' "$tmp/calls"
grep -Eq 'issue edit 1 --repo owner/repo --add-label codex' "$tmp/calls"
